...
class StoreServiceTest {
    ...
    void shouldReturnFiveClosestStoresInOrder() {
        ...
        when(repository.loadStore()).thenReturn(storesMock);

        List<Store> response = service.findClosest(51.456157, 5.804116);
        List<String> resultUuids = response.stream()
                .map(Store::uuid)
                .toList();
        ...
        verify(repository, times(1)).loadStore();
    }
